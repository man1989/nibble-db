...
var Collection = class _Collection {
  fullName;
  rootPath = join(__dirname, "userData");
  ...
  async insert(obj) {
    ...
    try {
      let fileData = await readFile(path, "utf-8") || "[]";
  ...
  async update(query, obj) {
    ...
    validateInput(obj);
    const filtered = filter(query);
    let fileData = await readFile(this.getPath(), "utf-8");
    try {
      let results = JSON.parse(fileData);
...
var DB = class _DB {
  static _rootPath = join2(__dirname, "../.$$db$$");
  ...
  static async create(name, rootPath) {
    ...
    try {
      await stat2(this._rootPath);
    } catch (err) {
      await mkdir(join2(this._rootPath, name), {
        recursive: true
      });
    }
    return new _DB(name);
